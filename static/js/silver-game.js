...
// Each pad = one visible bet (first 6)
...
function showSlotsFullPopup() {
  ...
  if (popupMsgEl)
    popupMsgEl.textContent = "This game is already full. You will be redirected to lobby.";
  popupEl.style.display = "flex";
  setTimeout(() => {
    window.history.back();
  }, 2000);
...
// Always animates frog to specified winning pad
function hopFrogToWinningNumber(winningNumber) {
  ...
  if (!targetPad) return;  // This is now always present due to ensurePadForWinningNumber
...
function updateGameUI(table) {
  ...
  if (placeBetBtn && !gameFinished) {
    placeBetBtn.disabled =
      ...
      (typeof table.max_players === "number" && table.players >= table.max_players);
  }
...
if (placeBetBtn) {
  placeBetBtn.addEventListener("click", () => {
    ...
    const maxPlayers = typeof currentTable.max_players === "number"
